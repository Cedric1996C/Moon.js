...
class Messager {
  master: CalabashCluster;
  constructor(master: CalabashCluster) {
    this.master = master;
  }
...
export default class CalabashCluster extends EventEmitter {
  ip: string = '127.0.0.1';
  port: number = 3001;
  ...
  onAppStart() {
    logger.green(`[master]#${process.pid} Calabash app started ${this.ip}:${this.port}`);
  }
  onAppExit() {
